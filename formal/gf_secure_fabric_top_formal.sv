// Formal-only version of gf_secure_fabric_top (Yosys compatible)
// Replaces SV interfaces with flat wires, unpacked arrays with packed.
// Identical chassis logic — only engine instantiation differs.
`include "gf_pkg.sv"
...
#(
  ...
  // Montgomery cluster (packed arrays for Yosys)
  logic             mc_req_valid;
  logic             mc_req_ready;
  logic [WIDTH-1:0] mc_a, mc_b, mc_m;
  logic             mc_rsp_valid;
  logic             mc_rsp_ready;
  logic [WIDTH-1:0] mc_p;
  logic             mc_idle;
  ...
  // ─── Frontend ────────────────────────────────────────────────────────────
  gf_axil_frontend #(.WIDTH(WIDTH), .EXP_BLIND_BITS(EXP_BLIND_BITS)) u_frontend (
    .clk_i, .rst_ni,
    ...
    .irq_o,
    .cmd_valid_o(cmdf_in_valid), .cmd_ready_i(cmdf_in_ready), .cmd_o(cmdf_in),
    .resp_valid_i(rf_out_valid), .resp_ready_o(rf_out_ready), .resp_i(rf_out),
    .hw_we_o(hw_we), .hw_bank_o(hw_bank), .hw_region_o(hw_region),
    .hw_word_o(hw_word), .hw_data_o(hw_data),
    .hr_bank_o(hr_bank), .hr_word_o(hr_word), .hr_data_i(hr_data),
    .retire_o(retire), .retire_bank_o(retire_bank),
    .rsa_p_o(rsa_p), .rsa_q_o(rsa_q), .rsa_dp_o(rsa_dp),
    .rsa_dq_o(rsa_dq), .rsa_qinv_o(rsa_qinv),
    .coeff_we_o(coeff_ld_we), .coeff_addr_o(coeff_ld_addr), .coeff_data_o(coeff_ld_data),
    .bank_free_i(bank_free), .fabric_busy_i(sched_busy),
  ...
  gf_fifo #(.DATA_W($bits(gf_cmd_t)), .DEPTH(MAX_TXNS)) u_command_fifo (
    .clk_i, .rst_ni,
    .valid_i(cmdf_in_valid), .ready_o(cmdf_in_ready), .data_i(cmdf_in),
    .valid_o(cmdf_out_valid), .ready_i(cmdf_out_ready), .data_o(cmdf_out),
    .count_o()
  );

  // ─── Operand banks ───────────────────────────────────────────────────────
  gf_operand_banks #(.WIDTH(WIDTH), .EXP_BLIND_BITS(EXP_BLIND_BITS)) u_operand_banks (
    .clk_i, .rst_ni,
    .hw_we_i(hw_we), .hw_bank_i(hw_bank), .hw_region_i(hw_region),
    .hw_word_i(hw_word), .hw_data_i(hw_data),
    .hr_bank_i(hr_bank), .hr_word_i(hr_word), .hr_data_o(hr_data),
    .rd_bank_i(bank_rd), .rd_a_o(bank_a), .rd_b_o(bank_b), .rd_m_o(bank_m),
    .res_we_i(res_we), .res_bank_i(res_bank), .res_data_i(res_data),
    .wipe_i(retire), .wipe_bank_i(retire_bank)
  );
  ...
  gf_transaction_table u_transaction_table (
    ...
    .ret_en_i(retire), .ret_slot_i(retire_bank),
    .comp_en_i(tt_comp_en), .comp_slot_i(tt_comp_slot), .comp_status_i(tt_comp_status),
    .state_o(tt_state), .txn_id_o(tt_txn_id), .opcode_o(tt_opcode),
    .seq_o(tt_seq), .status_o(tt_status)
  );

  // ─── Scheduler ───────────────────────────────────────────────────────────
  gf_scheduler #(.WIDTH(WIDTH), .EXP_BLIND_BITS(EXP_BLIND_BITS)) u_scheduler (
    .clk_i, .rst_ni,
    .cmd_valid_i(cmdf_out_valid), .cmd_ready_o(cmdf_out_ready), .cmd_i(cmdf_out),
    .bank_rd_o(bank_rd), .bank_a_i(bank_a), .bank_b_i(bank_b), .bank_m_i(bank_m),
    .e0_valid_o(e0_cmd_valid), .e0_ready_i(e0_cmd_ready),
    .e0_base_o(e0_base), .e0_exp_o(e0_exp), .e0_m_o(e0_m),
    .e0_done_valid_i(e0_done_valid), .e0_done_ready_o(e0_done_ready),
    .e0_result_i(e0_result), .e0_status_i(e0_status),
    .e1_valid_o(e1_cmd_valid), .e1_ready_i(e1_cmd_ready),
    ...
    .e2_valid_o(e2_cmd_valid), .e2_ready_i(e2_cmd_ready),
    ...
    .e3_valid_o(e3_cmd_valid), .e3_ready_i(e3_cmd_ready),
    ...
    .e4_valid_o(e4_cmd_valid), .e4_ready_i(e4_cmd_ready),
    ...
    .cq_valid_o(cq_in_valid), .cq_ready_i(cq_in_ready), .cq_data_o(cq_in),
    ...
    .tt_comp_status_o(tt_comp_status), .tt_state_i(tt_state),
    .bank_free_o(bank_free), .busy_o(sched_busy)
  );

  // ─── Engine 0: modexp (black-boxed) ──────────────────────────────────────
  gf_modexp_engine #(.WIDTH(WIDTH), .EXP_BLIND_BITS(EXP_BLIND_BITS)) u_modexp_engine (
    .clk_i, .rst_ni,
    .valid_i(e0_cmd_valid), .ready_o(e0_cmd_ready),
    .base_i(e0_base), .exp_i(e0_exp), .m_i(e0_m),
    .valid_o(e0_done_valid), .ready_i(e0_done_ready),
    .result_o(e0_result), .status_o(e0_status),
    .mul_req_valid_o(mc_req_valid), .mul_req_ready_i(mc_req_ready),
    .mul_a_o(mc_a), .mul_b_o(mc_b), .mul_m_o(mc_m),
    .mul_rsp_valid_i(mc_rsp_valid), .mul_rsp_ready_o(mc_rsp_ready),
    .mul_p_i(mc_p), .idle_o(e0_idle)
  );

  // ─── Engine 1: modinv (black-boxed) ──────────────────────────────────────
  gf_modinv_engine #(.WIDTH(WIDTH)) u_modinv_engine (
    .clk_i, .rst_ni,
    .valid_i(e1_cmd_valid), .ready_o(e1_cmd_ready),
    .a_i(e1_a), .m_i(e1_m),
    .valid_o(e1_done_valid), .ready_i(e1_done_ready),
    .result_o(e1_result), .status_o(e1_status), .idle_o(e1_idle)
  );

  // ─── Engines 2-4: stubs (black-boxed, immediate completion) ─────────────
  assign e2_cmd_ready = 1'b1;
  assign e2_done_valid = 1'b0;
  assign e2_result = '0;
  assign e2_status = STATUS_OK;
  assign e2_idle = 1'b1;

  assign e3_cmd_ready = 1'b1;
  assign e3_done_valid = 1'b0;
  assign e3_result = '0;
  assign e3_status = STATUS_OK;
  assign e3_idle = 1'b1;

  assign e4_cmd_ready = 1'b1;
  assign e4_done_valid = 1'b0;
  assign e4_result = '0;
  assign e4_status = STATUS_OK;
  assign e4_idle = 1'b1;

  assign pqc_mul_req_valid = 1'b0;
  assign pqc_mul_a = '0;
  assign pqc_mul_b = '0;
  assign pqc_mul_m = '0;
  assign pqc_mul_rsp_ready = 1'b1;
  assign eng_coeff_wr_en = 1'b0;
  assign eng_coeff_wr_addr = '0;
  assign eng_coeff_wr_data = '0;
  assign eng_coeff_rd_en = 1'b0;
  assign eng_coeff_rd_addr = '0;

  // ─── Montgomery cluster (black-boxed) ────────────────────────────────────
  gf_montgomery_cluster #(.WIDTH(WIDTH), .NUM_MULTIPLIERS(1)) u_montgomery_cluster (
    .clk_i, .rst_ni,
    .req_valid_i(mc_req_valid), .req_ready_o(mc_req_ready),
    .req_a_i(mc_a), .req_b_i(mc_b), .req_m_i(mc_m),
    .rsp_valid_o(mc_rsp_valid), .rsp_ready_i(mc_rsp_ready),
    .rsp_p_o(mc_p), .idle_o(mc_idle)
  );

  // ─── Completion path ─────────────────────────────────────────────────────
  gf_completion_queue u_completion_queue (
    .clk_i, .rst_ni,
    .valid_i(cq_in_valid), .ready_o(cq_in_ready), .data_i(cq_in),
  ...
  gf_reorder_buffer #(.RESPONSE_ORDERING(RESPONSE_ORDERING)) u_reorder_buffer (
    ...
    .valid_o(rob_out_valid), .ready_i(rob_out_ready), .data_o(rob_out)
  );

  gf_fifo #(.DATA_W($bits(gf_completion_t)), .DEPTH(MAX_TXNS)) u_result_fifo (
    .clk_i, .rst_ni,
    .valid_i(rob_out_valid), .ready_o(rob_out_ready), .data_i(rob_out),
    .valid_o(rf_out_valid), .ready_i(rf_out_ready), .data_o(rf_out),
    .count_o(rf_count)
  );
